...
parser.add_argument('--max_step', type=int, default=15, help="number of frames to predict")
...
if opt.model_dir != '':
    ...
    opt.model, opt.image_width, opt.image_width, opt.rnn_size, opt.predictor_rnn_layers, opt.posterior_rnn_layers,
    opt.prior_rnn_layers, opt.n_past, opt.n_future, opt.lr, opt.g_dim, opt.z_dim, opt.last_frame_skip, opt.beta,
    opt.name)
...
from generation_model import generation
from models.distribution import Posterior, Prior
posterior = Posterior(opt.g_dim+opt.rnn_size, opt.z_dim)
prior = Prior(opt.rnn_size, opt.z_dim)
...
frame_generator = generation(encoder, posterior, prior, decoder)
...
"""
optimizer = opt.optimizer(frame_generator.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
...
def plot_rec(x, epoch):
    ...
    for i in range(1, opt.n_past + opt.n_future):
        ...
        z_t, _, _ = posterior(h_target, init=(i == 1))
        if i < opt.n_past:
            frame_predictor(torch.cat([h, z_t], 1), init=(i == 1))
            gen_seq.append(x[i])
        else:
            h_pred = frame_predictor(torch.cat([h, z_t], 1), init=(i == opt.n_past))
...
def train(x):
    ...
    kld = 0
    for i in range(1, opt.max_step):
        x_pred, mu_p, logvar_p, mu, logvar = frame_generator(x[i-1], i-1)
...
for epoch in range(opt.niter):
    ...
    epoch, epoch_mse / epoch_size, epoch_kld / epoch_size, epoch * epoch_size * opt.batch_size))
